...
export interface PaymentRequest {
  pg: PG; // PG사 구분 코드 (예: "kakaopay.TC0ONETIME")
  pay_method: PaymentMethod; // 결제 방법 (예: "card", "trans", "vbank", "point" 등)
  merchant_uid: string; // 가맹점 주문 번호 (고유한 결제 ID)
  name: string; // 결제 상품명
  amount: number; // 결제 금액
  buyer_email: string; // 구매자 이메일
  buyer_name: string; // 구매자 이름
  m_redirect_url: string; // 모바일 웹 결제 후 리디렉트 URL (필요한 경우)
}

export interface PaymentResponse {
  buyer_email: string; // 구매자 이메일
  buyer_name: string; // 구매자 이름
  card_quota: number; // 할부 개월 수 (0이면 일시불)
  currency: string; // 결제 통화 (KRW, USD 등)
  imp_uid: string; // 포트원 결제 고유 ID
  merchant_uid: string; // 가맹점 주문 번호
  name: string; // 결제 상품명
  paid_amount: number; // 결제 금액
  paid_at: number; // 결제 완료 시간 (UNIX timestamp)
  pay_method: string; // 결제 방법 (card, trans, vbank, point 등)
  pg_provider: string; // PG사 이름 (kakaopay, toss 등)
  pg_tid: string; // PG사 거래 번호
  pg_type: string; // PG사 결제 타입 (payment, refund 등)

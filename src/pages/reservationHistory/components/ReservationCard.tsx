  ...
  return (
    <Card isCompleted={isCompleted}>
      <CardHeader>
        ...
        {status && (
          <StatusBadge color={statusColor} isCompleted={isCompleted}>
            {status}
          </StatusBadge>
        )}
      </CardHeader>
      ...
      {isCancellable && (
        <CancelButton onClick={() => onClick && onClick(paymentId)} />
      )}{" "}
      {/* ✅ onClick 사용 */}
...
const Card = styled.div<{ isCompleted?: boolean }>`
  ...
  margin-top: 10px;
  color: ${({ isCompleted }) =>
    isCompleted ? theme.colors.gray[300] : theme.colors.black};
`;
...
const StatusBadge = styled.span<{ color?: string; isCompleted?: boolean }>`
  font-size: 14px;
  color: ${({ isCompleted, color }) =>
    isCompleted ? theme.colors.gray[300] : color || theme.colors.gray[900]};
  font-weight: bold;
`;
